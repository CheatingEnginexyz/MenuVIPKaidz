...
document.addEventListener("DOMContentLoaded",function(){
  ...
  function buildNotice(){
    const items=[
{icon:"",text:"UY TÍN - CHẤT LƯỢNG - AN TOÀN - HIỆU QUẢ "},
{icon:"",text:"KHÔNG CAN THIỆP PHẦN MỀM THỨ 3 AN TOÀN 100%"},
{icon:"",text:"TINH CHỈNH NHANH CẤU HÌNH TRONG 1 CHẠM"},
    ];      
  ...
  function openMenu(){menu&&menu.classList.remove("hidden"); menu&&menu.setAttribute("aria-hidden","false"); toast("SUPPORT TECH ĐÃ MỞ"); if(!store.get("menuNoticeSeen",false)){openNotice(); store.set("menuNoticeSeen",true)}}
  ...
  if(fixRungToggle) fixRungToggle.addEventListener("change",()=>{store.set("fix-rung-enabled",fixRungToggle.checked); reflect(); runAction("fixRung",fixRungToggle.checked); toast(fixRungToggle.checked?"STEADY HOLD : ON":"STEADY HOLD : OFF")});
  if(nheTamToggle) nheTamToggle.addEventListener("change",()=>{store.set("nhe-tam-enabled",nheTamToggle.checked); reflect(); runAction("nheTam",nheTamToggle.checked); setCenterReticleHidden(nheTamToggle.checked); toast(nheTamToggle.checked?"ANCHOER AIM : ON":"ANCHOER AIM : OFF")});
...
(function(){
  var OFF_KEYS=["config-enabled","lux-enabled","fix-rung-enabled","nhe-tam-enabled","lock-dau-enabled","feat-anti-shake","feat-aim-assist","feat-touch-boost","feat-pro-mode"];
  var STATE_KEYS=["config-enabled","lux-enabled","fix-rung-enabled","nhe-tam-enabled","lock-dau-enabled"];
